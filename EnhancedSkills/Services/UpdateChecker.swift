...
enum UpdateChecker {
    static let githubRepo = "sameerbajaj/EnhancedSkills"
    ...
    static func check() async -> UpdateCheckResult {
        ...
        do {
            ...
            if httpResponse.statusCode == 404 {
                return .failed("The GitHub releases endpoint returned 404. The repository may be private or releases are not yet published.")
            }
            ...
            guard let releases = try? JSONDecoder().decode([GitHubRelease].self, from: data) else {
                return .failed("EnhancedSkills could not decode the GitHub releases response.")
            }
    ...
    static var buildTimestamp: TimeInterval {
        guard let value = Bundle.main.infoDictionary?["CFBundleVersion"] as? String,
              let timestamp = TimeInterval(value) else { return 0 }
        return timestamp
    }
...
private struct GitHubRelease: Decodable {
    ...
    init(from decoder: Decoder) throws {
        ...
        if let iso = try c.decodeIfPresent(String.self, forKey: .publishedAt) {
            publishedAt = ISO8601DateFormatter().date(from: iso)?.timeIntervalSince1970
        } else {
            publishedAt = nil
        }
    }
