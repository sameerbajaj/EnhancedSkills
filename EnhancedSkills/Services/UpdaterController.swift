...
final class UpdaterController {
    ...
    private func presentUpdateAlert(for update: UpdateInfo) {
        ...
        alert.informativeText = update.isRolling
            ? "A newer GitHub Actions build is available. EnhancedSkills can install it now and relaunch automatically."
            : "EnhancedSkills \(update.version) is available on GitHub. It can install now and relaunch automatically."
        ...
        case .alertSecondButtonReturn:
            if update.downloadURL != nil { NSWorkspace.shared.open(update.releaseURL) }
        default:
            break
        }
    }
